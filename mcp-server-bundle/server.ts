...
// Prototype Flow Tools
server.tool(
  "get_flow_start_points",
  "List prototype flow start points for the current page or a specific page",
  {
    pageId: z.string().optional().describe("Optional page ID; defaults to current page"),
  },
  async ({ pageId }: any) => {
    try {
      const result = await sendCommandToFigma("get_flow_start_points", { pageId });
      return {
        content: [
          {
            type: "text",
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting flow start points: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Set prototype start point on a frame or component
server.tool(
  "set_flow_start_point",
  "Set a prototype flow start point on the given node",
  {
    nodeId: z.string().describe("ID of the frame/component to set as start point"),
    name: z.string().optional().describe("Optional flow name; defaults to 'Flow'"),
  },
  async ({ nodeId, name }: any) => {
    try {
      const result = await sendCommandToFigma("set_flow_start_point", { nodeId, name });
      return {
        content: [
          {
            type: "text",
            text: `Set prototype start point on ${nodeId}: ${JSON.stringify(result)}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error setting flow start point: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Clear prototype start point(s)
server.tool(
  "clear_flow_start_point",
  "Clear prototype flow start points by page, node, name, or all",
  {
    pageId: z.string().optional().describe("Optional page ID; defaults to current page"),
    nodeId: z.string().optional().describe("Optional node ID to clear start point for"),
    name: z.string().optional().describe("Optional flow name to clear"),
    clearAll: z.boolean().optional().describe("If true, clears all start points on the page"),
  },
  async ({ pageId, nodeId, name, clearAll }: any) => {
    try {
      const result = await sendCommandToFigma("clear_flow_start_point", { pageId, nodeId, name, clearAll });
      return {
        content: [
          {
            type: "text",
            text: `Cleared prototype start point(s): ${JSON.stringify(result)}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error clearing flow start point(s): ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
...
// Add prototype navigation reaction (source -> destination)
server.tool(
  "add_prototype_navigation",
  "Add a prototype reaction NAVIGATE from source to destination (fallbacks to connector if reactions cannot be written)",
  {
    sourceNodeId: z.string().describe("ID of the source node (hotspot)"),
    destinationNodeId: z.string().describe("ID of the destination frame/screen"),
    triggerType: z.string().optional().describe("Trigger type, default 'ON_CLICK'"),
    preserveScrollPosition: z.boolean().optional().describe("Preserve scroll position on navigation")
  },
  async ({ sourceNodeId, destinationNodeId, triggerType, preserveScrollPosition }: any) => {
    try {
      const result = await sendCommandToFigma("add_prototype_navigation", {
        sourceNodeId,
        destinationNodeId,
        triggerType,
      ...
      return {
        content: [
          {
            type: "text",
            text: `Added prototype navigation: ${JSON.stringify(result)}`
          }
        ]
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error adding prototype navigation: ${error instanceof Error ? error.message : String(error)}`
          }
        ]
      };
    }
